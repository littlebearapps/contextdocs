...
| `docs/faq/faq.md` | Synced FAQ — emits Schema.org `FAQPage` JSON-LD on `littlebearapps.com/help/contextdocs/faq/` via the marketing-site sync (issue #25, renamed from `index.md` per #29). **Do not delete or move** — upstream build hard-fails. **Update only when** an answer goes stale, not on every release |
